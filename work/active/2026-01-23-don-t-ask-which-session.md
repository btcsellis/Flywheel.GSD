...
- [ ] Orange "advance" button no longer shows session selection modal
- [ ] Automatically detects existing tmux session matching pattern `flywheel-{project}-{workItemId}`
- [ ] If matching session exists, uses it directly without prompting
- [ ] If no matching session exists, creates a new one automatically
- [ ] If multiple matching sessions exist, picks first one found
